...
func (l *Implementation) NewMacaroon(args impl.NewMacaroonSpec) (impl.Macaroon, error) {
	...
	if err != nil {
		return impl.Macaroon{}, fmt.Errorf("mack.UnsafeRootMacaroon: %w", err)
	}
	for _, c := range args.Caveats {
		m, err = mackScheme.AddFirstPartyCaveat(&m, c.ID)
		if err != nil {
			return impl.Macaroon{}, fmt.Errorf("mack.AddFirstPartyCaveat: %w", err)
		}
	}
...
func (l *Implementation) NewMacaroons(args impl.NewMacaroonSpec) (impl.Macaroons, error) {
	...
	if err != nil {
		return impl.Macaroons{}, fmt.Errorf("mack.PrepareStack: %w", err)
	}
	return impl.Macaroons{Slice: &st}, nil
}

func (l *Implementation) newMacaroonsFromSpec(args impl.NewMacaroonSpec) ([]mack.Macaroon, error) {
	m, err := mackScheme.UnsafeRootMacaroon(args.Location, args.ID, args.RootKey)
	if err != nil {
		return nil, fmt.Errorf("mack.UnsafeRootMacaroon: %w", err)
	}
	macaroons := []mack.Macaroon{m}
	for _, c := range args.Caveats {
		if c.Location != "" && len(c.Key) != 0 {
			m, err = mackScheme.AddThirdPartyCaveat(&m, c.Key, c.ID, c.Location)
			if err != nil {
				return nil, fmt.Errorf("mack.AddThirdPartyCaveat: %w", err)
			}
			macaroons[0] = m
			var discharge []mack.Macaroon
		...
		m, err = mackScheme.AddFirstPartyCaveat(&m, c.ID)
		if err != nil {
			return nil, fmt.Errorf("mack.AddFirstPartyCaveat: %w", err)
		}
		macaroons[0] = m
	}
...
func (l *Implementation) VerifyMacaroon(key []byte, ms impl.Macaroons) (bool, error) {
	st := ms.Slice.(*mack.Stack)
	_, err := mackScheme.Verify(context.TODO(), key, *st)
	if err != nil {
		return false, fmt.Errorf("mack.Verify: %w", err)
	}
	return true, nil
}
