...
type EncryptionScheme interface {
	...
	Encrypt(out []byte, in []byte, key []byte) ([]byte, error)
	...
	Decrypt(out []byte, in []byte, key []byte) ([]byte, error)
}
...
func (s *Scheme) encrypt(out []byte, in []byte, key []byte) ([]byte, error) {
	...
	out = s.growBuffer(out, len(in)+s.overhead)
	out, err := s.enc.Encrypt(out[:0], in, key)
	if err != nil {
...
func (s *Scheme) decrypt(out []byte, in []byte, key []byte) ([]byte, error) {
	if len(in) < s.overhead {
		return nil, fmt.Errorf("%w: input buffer too small. need=%d, got=%d", ErrInvalidArgument, s.overhead, len(in))
	}
	if len(key) != s.keySize {
		return nil, fmt.Errorf("%w: invalid key size. need=%d, got=%d", ErrInvalidArgument, s.keySize, len(key))
	}
	out = s.growBuffer(out, len(in)-s.overhead)
	return s.enc.Decrypt(out[:0], in, key)
}
