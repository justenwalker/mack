...
var benchResult Macaroon //nolint:gochecknoglobals // sink for benchmarks to avoid compiler optimizations
...
func BenchmarkNew(b *testing.B) {
	...
	var err error
	for i := 0; i < b.N; i++ {
		m, err = sch.NewMacaroon(locStr, id, key, cavs...)
		if err != nil {
...
func (t testScheme) Encrypt(_ []byte, in []byte, _ []byte, _ []byte) ([]byte, error) {
	return in, nil
}

func (t testScheme) Decrypt(_ []byte, in []byte, _ []byte, _ []byte) ([]byte, error) {
	return in, nil
}
