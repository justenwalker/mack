// Package agecrypt implements an Encryptor and Decryptor based on age Identities/Recipients.
// A thirdparty.ThirdParty would then use the age.Recipient of the thirdparty.Discharger
// to encrypt the caveat key (cK) and predicate (caveat ID). Then the thirdparty.Discharger could decrypt with its age.Identity.
// This allows the caveat key (cK) to be communicated securely using public key crypto without needing the thirdparty.Discharger
// to persistently store the association of the cK with the caveat ID when a macaroon is discharged.
package agecrypt
